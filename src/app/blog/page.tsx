...
export default async function Blog() {
  ...
  return (
    <main style={{ position: 'relative', overflow: 'clip', minHeight: '100vh', background: '#f8fafc' }}>
      <div className="glow-bg" style={{ opacity: 0.1 }}></div>
      
      {/* Nav Minimalista Estilo Portal */}
      <nav style={{ padding: '1rem 0', borderBottom: '1px solid rgba(0,0,0,0.05)', background: 'rgba(255, 255, 255, 0.9)', backdropFilter: 'blur(10px)', position: 'sticky', top: 0, zIndex: 100 }}>
        <div className="container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ fontFamily: 'Outfit', fontWeight: 800, fontSize: '1.4rem', color: '#1e293b' }}>
          ...
          <Link href="/" className="btn" style={{ fontSize: '0.85rem', padding: '0.5rem 1rem', background: '#f1f5f9', color: '#1e293b', borderRadius: '6px' }}>Voltar ao Site</Link>
        </div>
      </nav>

      <section className="section" style={{ paddingTop: '5rem', background: '#fff' }}>
        <div className="container">
          <div style={{ textAlign: 'center', marginBottom: '5rem' }}>
            ...
            <h1 style={{ fontSize: '4rem', fontWeight: 900, marginBottom: '1.5rem', letterSpacing: '-2px', color: '#0f172a', fontFamily: 'Outfit' }}>{siteContent.blog.title}</h1>
            <p style={{ fontSize: '1.25rem', color: '#475569', maxWidth: '700px', margin: '0 auto', lineHeight: 1.6 }}>{siteContent.blog.subtitle}</p>
          </div>
